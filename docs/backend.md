...
SQLite es la fuente de verdad. Los IDs de `users`, `workspaces`, `invitations` y `notifications` usan CUID2 lowercase alfanumérico mediante `createId()` y se almacenan como `TEXT`; `memberships` usa `(userId, workspaceId)` como PK compuesta y `push_installations` usa el FID como PK. Tablas implementadas: `users` (incluye estado y hash/expiración de verificación de email), `workspaces` (soft delete `deletedAt`, checks `name 3-50`, `description ≤1000`, indexes `ownerId/deletedAt`), `memberships` (PK compuesta, `role` check, indexes), `invitations` (tokenHash unique, expiración 7d, partial unique pendiente), `refresh_tokens` (tokenHash como PK, familia CUID2 por dispositivo, expiración y revocación), `notifications` (destinatario, tipo, recurso, snapshots de contenido, lectura e índices) y `push_installations` (FID, usuario, familia refresh y timestamps). Drizzle gestiona schema y migraciones versionadas. La base local debe sincronizarse antes de probar auth y notificaciones. Redis se usa para jobs temporales de BullMQ, nunca como fuente durable.
