...
- `backend`: API REST Hono ejecutada con Bun y persistencia SQLite mediante Drizzle. Expone `/api/v1/auth/*` (+ `GET /me`) y `/api/v1/workspaces` (filtrado por `memberships` + soft delete).
